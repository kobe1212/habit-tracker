...
// Variants (shadcn color tokens mapped to this app's theme tokens)
const streakBadgeVariants = cva(
  "inline-flex flex-col items-center justify-center rounded-3xl border border-line bg-surface text-center text-fg transition-colors",
  {
    variants: {
      size: {
        sm: "w-28 gap-1.5 p-3",
  ...
  (
    ...
    return (
      ...
      >
        {icon ?? (
          <Flame
            className={cn(iconSize, "text-brand shrink-0")}
            aria-hidden="true"
          />
        )}
        <span
        ...
        >
          {streakLength}
          <span className="text-muted ml-2 font-medium">{valueUnit}</span>
        </span>
        <span
          className={cn("text-muted font-normal", subtitleSize)}
          aria-hidden="true"
        >
          {subtitleText}
        </span>
